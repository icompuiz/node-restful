...
// Pivcon-Start: Define an alternate method for .register as .register gets obscured by passport mongoose module.
Model.nodeRestfulRegister = function(app, url) {
  this.addDefaultRoutes();
  app.getDetail = app.get;
  this.registerRoutes(app, url, '', this.routes);
}
...
function preprocess(req, res, next) {
  ...
  // pivcon-start: Add support for ?distinct=<field>
  if(req.query.distinct) {
    req.quer = this.filter(req, this.distinct(req.query.distinct));
  } else {
...
function filterable(props, subfilters) {
  return {
    filter: function(key, val, quer) {
      ...
      // pivcon-start: Add support for ?field=null to find records without a field
      if ((filter_func === 'equals') && (data === "null")) { data = null; }
      // pivcon-end
